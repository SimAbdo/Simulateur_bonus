...
from utils import (SYSTEMES_DEFAUT, calculer_primes_df, valider_donnees,
                   get_download_link, exporter_systeme_json, importer_systeme_json)
...
def page_configuration_systemes():
    ...
    with tab1:
        ...
        systeme_actuel = st.session_state.systemes_personnalises["systeme_actuel"]
        ...
        systeme_actuel["description"] = st.text_area("Description", 
        ...
        for idx, palier in paliers_df.iterrows():
            ...
            with col2:
                ...
                max_val = 1000000
                if not pd.isna(palier["max_voyageurs"]) and palier["max_voyageurs"] != float('inf'):
                ...
                paliers_df.at[idx, "max_voyageurs"] = max_v if max_v < 1000000 else float('inf')
            ...
            with col4:
                if st.button("🗑️", key=f"suppr_actuel_{idx}"):
                    ...
                    st.session_state.systemes_personnalises["systeme_actuel"]["paliers"] = paliers_df.values.tolist()
                    st.rerun()
        ...
        if st.button("➕ Ajouter un palier", key="add_actuel"):
            ...
            nouveau_palier = {"min_voyageurs": nouveau_min, 
                             "max_voyageurs": float('inf'), 
                             "taux": 0.1}
            paliers_df = pd.concat([paliers_df, pd.DataFrame([nouveau_palier])], ignore_index=True)
            st.session_state.systemes_personnalises["systeme_actuel"]["paliers"] = paliers_df.values.tolist()
            st.rerun()
    ...
    with tab2:
        ...
        systeme_nouveau = st.session_state.systemes_personnalises["systeme_nouveau"]
        ...
        systeme_nouveau["description"] = st.text_area("Description", 
        ...
        for idx, palier in paliers_df.iterrows():
            ...
            with col4:
                if st.button("🗑️", key=f"suppr_nouveau_{idx}"):
                    paliers_df = paliers_df.drop(idx).reset_index(drop=True)
                    st.session_state.systemes_personnalises["systeme_nouveau"]["paliers"] = paliers_df.values.tolist()
                    st.rerun()
    ...
    with col1:
        if st.button("Réinitialiser aux valeurs par défaut"):
            ...
            st.success("Les systèmes ont été réinitialisés aux valeurs par défaut.")
            st.rerun()
    
    with col2:
        if st.button("Appliquer les modifications"):
            st.success("Les modifications des systèmes de prime ont été appliquées.")
...
def page_principale():
    ...
    if st.session_state.data is not None:
        ...
        analyses['totaux_globaux'][f"cout_total_{systeme_base}"] = df_resultat[f"cout_total_{systeme_base}"].sum()
        analyses['totaux_globaux'][f"cout_total_{systeme_comp}"] = df_resultat[f"cout_total_{systeme_comp}"].sum()
        ...
        analyses['totaux_globaux'][f"bonus_cond_jour_{systeme_base}"] = (
            df_resultat[f"BONUS/CONDUCTEUR/J_{systeme_base}"] * df_resultat['NBRE CONDUCTEURS ETP']).sum() / total_conducteurs
        analyses['totaux_globaux'][f"bonus_cond_jour_{systeme_comp}"] = (
            df_resultat[f"BONUS/CONDUCTEUR/J_{systeme_comp}"] * df_resultat['NBRE CONDUCTEURS ETP']).sum() / total_conducteurs
            
        # Par mois (jour * 30)
        analyses['totaux_globaux'][f"bonus_cond_mois_{systeme_base}"] = analyses['totaux_globaux'][f"bonus_cond_jour_{systeme_base}"] * 30
        analyses['totaux_globaux'][f"bonus_cond_mois_{systeme_comp}"] = analyses['totaux_globaux'][f"bonus_cond_jour_{systeme_comp}"] * 30
        
        # Par an (jour * 365)
        analyses['totaux_globaux'][f"bonus_cond_an_{systeme_base}"] = analyses['totaux_globaux'][f"bonus_cond_jour_{systeme_base}"] * 365
        analyses['totaux_globaux'][f"bonus_cond_an_{systeme_comp}"] = analyses['totaux_globaux'][f"bonus_cond_jour_{systeme_comp}"] * 365

        # Calculer les différences
        diff_cout_total = analyses['totaux_globaux'][f"cout_total_{systeme_comp}"] - analyses['totaux_globaux'][f"cout_total_{systeme_base}"]
        diff_cout_total_pct = (diff_cout_total / analyses['totaux_globaux'][f"cout_total_{systeme_base}"] * 100
                              if analyses['totaux_globaux'][f"cout_total_{systeme_base}"] != 0 else 0)
        ...
        with col1:
            ...
            st.metric("Nombre total de voyageurs par an", f"{int(analyses['totaux_globaux']['VOY_TOTAL']):,}")
        
        with col2:
            st.metric(f"Somme totale des bonus - {systemes_actifs[0]['nom']}", 
                     f"{analyses['totaux_globaux'][f'cout_total_{systeme_base}']:,.2f} MAD")
            st.metric(f"Bonus moyen par conducteur/jour - {systemes_actifs[0]['nom']}", 
                     f"{analyses['totaux_globaux'][f'bonus_cond_jour_{systeme_base}']:,.2f} MAD")
            st.metric(f"Bonus moyen par conducteur/mois - {systemes_actifs[0]['nom']}", 
                     f"{analyses['totaux_globaux'][f'bonus_cond_mois_{systeme_base}']:,.2f} MAD")
            st.metric(f"Bonus moyen par conducteur/an - {systemes_actifs[0]['nom']}", 
                     f"{analyses['totaux_globaux'][f'bonus_cond_an_{systeme_base}']:,.2f} MAD")
        
        with col3:
            st.metric(f"Somme totale des bonus - {systemes_actifs[1]['nom']}", 
                     f"{analyses['totaux_globaux'][f'cout_total_{systeme_comp}']:,.2f} MAD")
            st.metric(f"Bonus moyen par conducteur/jour - {systemes_actifs[1]['nom']}", 
                     f"{analyses['totaux_globaux'][f'bonus_cond_jour_{systeme_comp}']:,.2f} MAD")
            st.metric(f"Bonus moyen par conducteur/mois - {systemes_actifs[1]['nom']}", 
                     f"{analyses['totaux_globaux'][f'bonus_cond_mois_{systeme_comp}']:,.2f} MAD")
            st.metric(f"Bonus moyen par conducteur/an - {systemes_actifs[1]['nom']}", 
                     f"{analyses['totaux_globaux'][f'bonus_cond_an_{systeme_comp}']:,.2f} MAD")
        
        # Afficher la différence en pourcentage
        st.info(f"**Différence entre les systèmes**: {diff_cout_total:,.2f} MAD ({diff_cout_total_pct:.2f}%)")
        ...
        chart = alt.Chart(df_chart).mark_bar().encode(
            ...
            tooltip=['Système', 'Bonus total (MAD)']
        ).properties(
        ...
        chart_data_cond = pd.DataFrame({
            ...
            'Système': [systemes_actifs[0]['nom']] * 3 + [systemes_actifs[1]['nom']] * 3,
            'Bonus moyen (MAD)': [
                analyses['totaux_globaux'][f"bonus_cond_jour_{systeme_base}"],
        ...
        chart_cond = alt.Chart(chart_data_cond).mark_bar().encode(
            ...
            tooltip=['Système', 'Période', 'Bonus moyen (MAD)']
        ).properties(
            title='Comparaison des bonus moyens par conducteur',
            width=150
        )
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            df_resultat[colonnes_affichage].to_excel(writer, sheet_name='Resultats', index=False)
            
            # Créer un onglet pour les KPIs
            kpi_df = pd.DataFrame({
                'Métrique': [
                    "Nombre de lignes",
                    "Nombre total de voyageurs par an",
            ...
            sys_actuel_df = pd.DataFrame(systemes_actifs[0]['paliers'], 
                                        columns=["Min Voyageurs", "Max Voyageurs", "Taux (MAD)"])
            sys_actuel_df.to_excel(writer, sheet_name=f'Système {systemes_actifs[0]["nom"]}', index=False)
            
            sys_nouveau_df = pd.DataFrame(systemes_actifs[1]['paliers'], 
                                         columns=["Min Voyageurs", "Max Voyageurs", "Taux (MAD)"])
            sys_nouveau_df.to_excel(writer, sheet_name=f'Système {systemes_actifs[1]["nom"]}', index=False)
        ...
        with col1:
            st.markdown(f"### {st.session_state.systemes_personnalises['systeme_actuel']['nom']}")
            st.write(st.session_state.systemes_personnalises["systeme_actuel"]["description"])

            # Afficher les paliers sous forme de tableau
            paliers_df = pd.DataFrame(st.session_state.systemes_personnalises["systeme_actuel"]["paliers"])
            paliers_df.columns = ["Min Voyageurs", "Max Voyageurs", "Taux (MAD)"]
            st.dataframe(paliers_df)

        with col2:
            st.markdown(f"### {st.session_state.systemes_personnalises['systeme_nouveau']['nom']}")
            st.write(st.session_state.systemes_personnalises["systeme_nouveau"]["description"])

            # Afficher les paliers sous forme de tableau
            paliers_df = pd.DataFrame(
                st.session_state.systemes_personnalises["systeme_nouveau"]["paliers"])
            paliers_df.columns = ["Min Voyageurs", "Max Voyageurs", "Taux (MAD)"]
            st.dataframe(paliers_df)
